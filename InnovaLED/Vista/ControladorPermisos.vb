Public Class ControladorPermisos
    ...
    Public Shared Sub CargarPermisos(ByRef Tree As TreeView, Optional Permisos As Entidades.RolEntidad = Nothing)
        Try
    ...
    Public Shared Function RecorrerArbol(ByRef arbol As TreeNode, ByRef Compuesto As Entidades.RolEntidad, ByRef Tree As TreeView) As Entidades.RolEntidad
        If IsNothing(arbol) Then
            For Each nodo As TreeNode In Tree.Nodes
                If nodo.Checked = True Then
                    'If nodo.ChildNodes.Count > 0 Then
                    '    Dim Permiso As New Entidades.RolEntidad With {.ID_Rol = nodo.Value, .Nombre = nodo.Text}
                    ...
                    'Else
                    Dim Permiso As New Entidades.PermisoBaseEntidad With {.ID_Permiso = nodo.Value, .Nombre = nodo.Text}
                    If Not Permiso.esValido(Compuesto.Nombre) Then
                        Compuesto.agregarHijo(Permiso)
                    End If
                    'End If
        ...
        Else
            For Each nodo2 As TreeNode In arbol.ChildNodes
                If nodo2.Checked = True Then
                    ...
                    'Dim Permiso As New Entidades.PermisoBaseEntidad With {.ID_Rol = nodo2.Value, .Nombre = nodo2.Text}
                    'If Not Permiso.esValido(Compuesto.Nombre) Then
                    ...
                    ''End If
                    ''Else
                    Dim Permiso As New Entidades.PermisoBaseEntidad With {.ID_Permiso = nodo2.Value, .Nombre = nodo2.Text}
                    If Not Permiso.esValido(Compuesto.Nombre) Then
                        Compuesto.agregarHijo(Permiso)
                    End If
                    'End If
